...
class ExpressionInSelect extends ExpressionArithmetic {
	...
	ExpressionInSelect(SSConnection con, Expression left, CommandSelect cmdSel,
			int operation) {
	...
	private void loadInList() throws Exception {
		if (cmdSel.compile(con)) {
			...
			if (cmdSel.columnExpressions.size() != 1)
				throw SmallSQLException.create(Language.SUBQUERY_COL_COUNT,
						new Integer(cmdSel.columnExpressions.size()));
			index.clear();
			while (cmdSel.next()) {
				try {
