...
class MutableNumeric extends Number implements Mutable {
	...
	MutableNumeric(double val) {
		// first convert it to a string, because double to BigDecimal has very
		// large rounding bug
		this(new BigDecimal(String.valueOf(val)));
	}

	MutableNumeric(float val) {
		// first convert it to a string, because float to BigDecimal has very
		// large rounding bug
		this(new BigDecimal(String.valueOf(val)));
	}
	...
	/**
	 * Add the value to the current MutableNumeric Object and change it. The
	 * parameter <code>val2</code> has a shorter or equals length. The signum of
	 * both values is equals.
	 * 
	 * @param val2
	 *            the added value
	 */
	private void add(int[] val2) {
		...
		for (int v2 = val2.length; v2 > 0;) {
			temp = (value[--v1] & 0xFFFFFFFFL) + (val2[--v2] & 0xFFFFFFFFL)
					+ (temp >>> 32);
			value[v1] = (int) temp;
		}
	 ...
	 * 
	 * @param highBits
	 *            Is the high value that is save on the resize place.
	 */
	...
	/**
	 * Subtract the value to the current MutableNumeric Object and change it.
	 * The parameter <code>val2</code> has a shorter or equals length. The
	 * signum of both values is equals.
	 * 
	 * @param val2
	 *            the subtracted value
	 */
	...
	void div(MutableNumeric num) {
		...
		BigDecimal big = toBigDecimal().divide(num.toBigDecimal(), newScale,
	...
	private void negate(byte[] complement) {
		...
		for (int i = 0; i <= last; i++) {
			complement[i] = (byte) ((i == last) ? -complement[i]
					: ~complement[i]);
		}
	...
	byte[] toByteArray() {
		...
		int v = 0;
		while (v < value.length && value[v] == 0)
			v++;
		if (v == value.length)
			return EMPTY_BYTES;
		...
		int last = value.length - 1;
		for (; v <= last; v++) {
			int val = (signum > 0) ? value[v] : (v == last) ? -value[v]
					: ~value[v];
	...
	public long longValue() {
		if (value.length == 0 || signum == 0) {
			...
			if (value.length == 1 && (value[0] > 0)) {
				...
				return Utils.double2long(value[0] / scaleDoubleFactor[scale]
						* signum);
			} else if (value.length == 1) {
				// overflow Integer Value
				long temp = value[0] & 0xFFFFFFFFL;
				return Utils.double2long(temp / scaleDoubleFactor[scale]
						* signum);
			} else if (value.length == 2 && (value[0] > 0)) {
				// simple Long Value
				long temp = (((long) value[0]) << 32)
						| (value[1] & 0xFFFFFFFFL);
				return Utils.double2long(temp / scaleDoubleFactor[scale]
						* signum);
			} else {
	...
	private static final double[] scaleDoubleFactor = { 1, 10, 100, 1000,
			10000, 100000, 1000000, 10000000 };
	private static final float[] scaleFloatFactor = { 1, 10, 100, 1000, 10000,
