...
public class StoreImpl extends Store {
	...
	private static final int PAGE_MAGIC = 0x12DD13DE; // are used for repairing
														// a table
	/**
	 * The structure of the Page Control Block is: 4 byte - page magic 4 Byte -
	 * Status 0:normal; 1:deleted; 2:Pointer to an update; 3: updated page 4
	 * Byte - used size of the page 4 byte - physical size of the page 4 byte -
	 * offset to the next page 8 byte - position of an updated page
	 */
	private static final int PAGE_CONTROL_SIZE = 28;
	private static final byte[] page_control = new byte[PAGE_CONTROL_SIZE];
	private static final ByteBuffer pageControlBuffer = ByteBuffer
			.wrap(page_control);
	private int status; // valid value are follow:
	private static final int NORMAL = 0;
	private static final int DELETED = 1;
	/**
	 * Using of UPDATE_POINTER and UPDATED_PAGE If a page are updated and the
	 * new data are larger as the old data then the old page are changed to a
	 * UPDATE_POINTER. The new page is a UPDATED_PAGE. On reading the pages only
	 * the UPDATE_POINTER is read. The UPDATED_PAGE are skipped. Thats a row
	 * change not it position.
	 */
	...
	private byte[] page;
	/**
	 * if the current array "page" is shared. This make sence for read only
	 * access but not if it will be write.
	 */
	...
	/**
	 * Follow types of StoreImpl are possible: INSERT: A page that will be
	 * include new data. filePos is not define yet. CREATE: A special type of
	 * INSERT SELECT: Only read operations are possible. UPDATE: Has a filePos,
	 * if the new size is to small then the old page must be deleted and a new
	 * added. DELETE: Has no cache else only the filePos to write the flag.
	 */
	static StoreImpl createStore(Table table, StorePage storePage, int type,
			long filePos) throws SQLException {
		try {
	 ...
	 */
	static StoreImpl recreateStore(Table table, StorePage storePage, int type)
			throws Exception {
	...
	private final void readPageHeader() throws SQLException {
		if (readInt() != PAGE_MAGIC) {
			throw SmallSQLException.create(Language.TABLE_CORRUPT_PAGE,
					new Object[] { new Long(filePos) });
		}
		status = readInt();
		sizeUsed = readInt();
	...
	/**
	 * If the current page is only an pointer to a larger page then the larger
	 * page is loaded else it return this.
	 * 
	...
	final private StoreImpl loadUpdatedStore() throws Exception {
		if (status != UPDATE_POINTER)
			return this;
		StoreImpl storeTemp = table.getStore(((TableStorePage) storePage).con,
				filePosUpdated, type);
	...
	boolean isValidPage() {
		return status == NORMAL
				|| (status == UPDATED_PAGE && updatePointer != null);
	}
	...
	long getNextPagePos() {
		if (updatePointer != null)
			return updatePointer.getNextPagePos();
		if (nextPageOffset <= 0) {
	 ...
	 * 
	 * @param con
	 *            Is needed to add this page to the commitPages. If it null then
	 *            it save directly without rollback option.
	...
	long writeFinsh(SSConnection con) throws SQLException {
		...
		storePage.setPageData(page, sizeUsed); // TODO page sollte eigentlich
												// beim einlesen gesetzt sein
		if (con == null) {
	...
	/**
	 * Request a write lock for the current read page. It add the resulting
	 * StorePage to the list of commits.
	 * 
	 * @throws SQLException
	 *             if for the current row (page) cannot create a write lock or
	 *             th connection was closed.
	 */
	final void createWriteLock() throws SQLException {
		TableStorePage storePageWrite = table.requestWriteLock(
				((TableStorePage) storePage).con, (TableStorePage) storePage);
	...
	/**
	 * Is call from updateRow(). The offset of newData must be at the end of the
	 * data. It used as new page size.
	 */
	void updateFinsh(SSConnection con, StoreImpl newData) throws SQLException {
		...
		if (newData.offset <= sizePhysical || filePos == -1) {
			...
			if (sizePhysical < offset)
				sizePhysical = offset; // occur only on updates of not committed
										// inserts (filePos == -1
			writeFinsh(con);
		} else {
			...
			if (updatePointer == null) {
				// we need to create a new page and change the old page to a
				// UPDATE_POINTER
	...
	int readInt() {
		return ((page[offset++]) << 24) | ((page[offset++] & 0xFF) << 16)
				| ((page[offset++] & 0xFF) << 8) | ((page[offset++] & 0xFF));
	}
	...
	void writeString(String strDaten, int lengthColumn, boolean varchar)
			throws SQLException {
	...
	void writeBinary(byte[] daten, int lengthColumn, boolean varBinary)
			throws SQLException {
		int length = daten.length;

		if (lengthColumn < length) {
			Object params = new Object[] { new Integer(length),
					new Integer(lengthColumn) };
	...
	void writeLongBinary(byte[] daten) throws Exception {
		StoreImpl store = table.getLobStore(((TableStorePage) storePage).con,
				daten.length + 4, SQLTokenizer.LONGVARBINARY);
	...
	byte[] readLongBinary() throws Exception {
		...
		StoreImpl store = table.getLobStore(((TableStorePage) storePage).con,
				lobFilePos, SQLTokenizer.SELECT);
	...
	void writeLongString(String daten) throws Exception {
		...
		StoreImpl store = table.getLobStore(((TableStorePage) storePage).con,
				chars.length * 2L + 4, SQLTokenizer.LONGVARBINARY);
	...
	String readLongString() throws Exception {
		...
		StoreImpl store = table.getLobStore(((TableStorePage) storePage).con,
				lobFilePos, SQLTokenizer.SELECT);
		if (store == null)
			throw SmallSQLException.create(Language.LOB_DELETED);
	 ...
	 * @param tableFormatVersion
	 *            the file version of the table.
	 * @return
	 * @throws Exception
	 */
	...
	void copyValueFrom(StoreImpl store, int valueOffset, int length) {
		System.arraycopy(store.page, valueOffset, this.page, this.offset,
				length);
		this.offset += length;
	}

	/**
	 * Here are pass only Expression instead of Objects to optimize it for
	 * native data (no object). The value of offset must be correctly.
	 * 
	 * @param expr
	 *            the value that should be written
	 * @param column
	 *            a description of the target column
	 * @throws Exception
	...
	void writeExpression(Expression expr, Column column) throws Exception {
		...
		if (isNull && !column.isNullable()) {
			throw SmallSQLException.create(Language.VALUE_NULL_INVALID,
					column.getName());
		}
		...
		case SQLTokenizer.BINARY:
		case SQLTokenizer.VARBINARY:
			writeBinary(expr.getBytes(), column.getPrecision(),
		...
		case SQLTokenizer.LONGVARCHAR:
			...
			break;
		case SQLTokenizer.JAVA_OBJECT:
			// FIXME a MemoryStream can be faster because there are no
		...
		case SQLTokenizer.UNIQUEIDENTIFIER:
			...
			case SQLTokenizer.BLOB:
				...
				if (bytes.length != 16)
					throw SmallSQLException.create(
							Language.BYTEARR_INVALID_SIZE,
	...
	boolean getBoolean(int valueOffset, int dataType) throws Exception {
		...
		default:
			throw SmallSQLException
					.create(Language.VALUE_CANT_CONVERT, new Object[] {
							SQLTokenizer.getKeyWord(dataType), "BOOLEAN" });
		}
	}
	...
	int getInt(int valueOffset, int dataType) throws Exception {
		...
		case SQLTokenizer.JAVA_OBJECT:
			ByteArrayInputStream bais = new ByteArrayInputStream(
					readLongBinary());
			ObjectInputStream ois = new ObjectInputStream(bais);
			return ExpressionValue.getInt(ois.readObject().toString(),
		...
		default:
			throw SmallSQLException.create(Language.VALUE_CANT_CONVERT,
					new Object[] { SQLTokenizer.getKeyWord(dataType), "INT" });
		}
	}
	...
	long getLong(int valueOffset, int dataType) throws Exception {
		...
		case SQLTokenizer.JAVA_OBJECT:
			ByteArrayInputStream bais = new ByteArrayInputStream(
					readLongBinary());
			ObjectInputStream ois = new ObjectInputStream(bais);
			return ExpressionValue.getLong(ois.readObject().toString(),
					SQLTokenizer.VARCHAR);
		case SQLTokenizer.LONGVARBINARY:
		case SQLTokenizer.BLOB:
		...
		default:
			throw SmallSQLException
					.create(Language.VALUE_CANT_CONVERT, new Object[] {
							SQLTokenizer.getKeyWord(dataType), "BIGINT" });
		}
	}
	...
	float getFloat(int valueOffset, int dataType) throws Exception {
		...
		default:
			throw SmallSQLException.create(Language.VALUE_CANT_CONVERT,
					new Object[] { SQLTokenizer.getKeyWord(dataType), "REAL" });
		}
	}
	...
	double getDouble(int valueOffset, int dataType) throws Exception {
		...
		default:
			throw SmallSQLException
					.create(Language.VALUE_CANT_CONVERT, new Object[] {
							SQLTokenizer.getKeyWord(dataType), "NUMERIC" });
		}
	}
	...
	long getMoney(int valueOffset, int dataType) throws Exception {
		...
		case SQLTokenizer.SMALLDATETIME:
			throw SmallSQLException
					.create(Language.VALUE_CANT_CONVERT, new Object[] {
							SQLTokenizer.getKeyWord(dataType), "MONEY" });
		default:
			throw new Error();
		}
	}
	...
	MutableNumeric getNumeric(int valueOffset, int dataType) throws Exception {
		...
		case SQLTokenizer.BIT:
		case SQLTokenizer.BOOLEAN:
			return readBoolean() ? new MutableNumeric(1)
					: new MutableNumeric(0);
		case SQLTokenizer.BINARY:
		case SQLTokenizer.VARBINARY:
		...
		case SQLTokenizer.SMALLDATETIME:
			throw SmallSQLException
					.create(Language.VALUE_CANT_CONVERT, new Object[] {
							SQLTokenizer.getKeyWord(dataType), "NUMERIC" });
			// SAVER: check if this should be a kind of "type not recognized"
			// exception.
		default:
			throw new Error();
		}
	...
	void scanObjectOffsets(int[] offsets, int dataTypes[]) {
		...
		for (int i = 0; i < offsets.length; i++) {
			...
			case SQLTokenizer.VARBINARY:
				...
				if (!isNull)
					offset += count; // not use += directly because readShort
										// change the offset
				break;
			case SQLTokenizer.NUMERIC:
			case SQLTokenizer.DECIMAL:
			...
			case SQLTokenizer.NVARCHAR:
				...
				if (!isNull)
					offset += count << 1; // not use += directly because
											// readShort change the offset
				break;
			case SQLTokenizer.UNIQUEIDENTIFIER:
				offset += 16;
				break;
			default:
				throw new Error(String.valueOf(dataTypes[i]));
			}
		}
	}
