...
public final class DateTime implements Mutable {
	...
	static final int[] MONTH_DAYS = { 0, 31, 59, 90, 120, 151, 181, 212, 243,
			273, 304, 334 };

	private static final String[] SHORT_MONTHS = new DateFormatSymbols()
			.getShortMonths();

	/*
	 * unused code DateTime(final int year, final int month, final int day,
	...
	static long calcMillis(Details details) {
		return calcMillis(details.year, details.month, details.day,
				details.hour, details.minute, details.second, details.millis);
	}

	static long calcMillis(int year, int month, final int day, final int hour,
	...
	/**
	 * @param dataType
	 *            is used for the toString() method
	 */
	...
	static long parse(final String datetime) throws SQLException {
		try {
			...
			if (idx2 > 0) {
				...
				idx2 = datetime.indexOf(':', idx1);
				minute = Integer
						.parseInt(datetime.substring(idx1, idx2).trim());
			...
			if (month >= 12) {
				throw SmallSQLException.create(Language.MONTH_TOOLARGE,
						datetime);
			}
			if (day >= 32) {
				throw SmallSQLException
						.create(Language.DAYS_TOOLARGE, datetime);
			}
			if (day == 31) {
				switch (month) {
				case 1:
				...
				case 8:
				case 10:
					throw SmallSQLException.create(Language.DAYS_TOOLARGE,
							datetime);
				}
			}
			if (month == 1) {
			...
			if (hour >= 24) {
				throw SmallSQLException.create(Language.HOURS_TOOLARGE,
						datetime);
			}
			if (minute >= 60) {
				throw SmallSQLException.create(Language.MINUTES_TOOLARGE,
						datetime);
			}
			if (second >= 60) {
				throw SmallSQLException
						.create(Language.SECS_TOOLARGE, datetime);
			}
			if (millis >= 1000) {
				throw SmallSQLException.create(Language.MILLIS_TOOLARGE,
						datetime);
			}
			...
			throw SmallSQLException.createFromException(
					Language.DATETIME_INVALID, datetime, ex);
		}
	}
	...
	/**
	 * Convert the this value in a String depending of the type the format is a
	 * subset of yyyy-mm-dd hh:mm:ss.nnn
	 */
	@Override
	...
	/**
	 * @param style
	 *            a value like the syle of CONVERT function from MS SQL Server.
	 */
	...
	private final static void formatNumber(int value, int digitCount,
			StringBuffer buf) {
	...
	private static long removeDateTimeOffset(long datetime) {
		synchronized (cal) {
			...
			return datetime + cal.get(Calendar.ZONE_OFFSET)
