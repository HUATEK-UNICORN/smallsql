...
public class ExpressionFunctionConvert extends ExpressionFunction {
	...
	public ExpressionFunctionConvert(Column datatype, Expression value,
			Expression style) {
		super();
		this.datatype = datatype;
		Expression[] params = (style == null) ? new Expression[] { value }
				: new Expression[] { value, style };
		setParams(params);
	}
	...
	Object getObject() throws Exception {
		...
		case SQLTokenizer.VARBINARY: {
			...
			if (length < bytes.length) {
				...
				System.arraycopy(bytes, 0, buffer, 0,
		...
		case SQLTokenizer.DECIMAL:
			...
			if (num != null
					&& (dataType == SQLTokenizer.NUMERIC || dataType == SQLTokenizer.DECIMAL))
		...
		case SQLTokenizer.UNIQUEIDENTIFIER:
			...
			case SQLTokenizer.CLOB:
				return Utils.bytes2unique(
						Utils.unique2bytes(param1.getString()), 0);
			}
	...
	final private String convertToString() throws Exception {
		if (param2 != null) {
			...
			case SQLTokenizer.DATE:
			case SQLTokenizer.TIME:
				return new DateTime(param1.getLong(), type).toString(param2
						.getInt());
			default:
