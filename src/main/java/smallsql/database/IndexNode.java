...
class IndexNode {
	...
	/**
	 * Can be a PageIndex to the next page or a byte[] with rest data of the
	 * key.
	 */
	private IndexNode[] nodes = EMPTY_NODES;

	/**
	 * On this point of the tree there is no other value. There is only one
	 * value. That's the tree is cut here and the single value is saved. This is
	 * very large benefit if you have large strings and you can difference it
	 * with the first characters.
	 */
	private char[] remainderKey;

	/**
	 * Can save a Long, LongList with a rowOffset value or a IndexNode of the
	 * next root index.
	 */
	private Object value;

	/**
	 * The status array save the status of the digits. The follow table descript
	 ...
	 * 
	 * @param unique
	 *            descript if it is an unique index (primary key) or a multi
	 *            value index is.
	 * @see #createIndexNode
	 */
	...
	/**
	 * Create a new Node in the Index. This is a factory method that must be
	 * overridden from extended classes.
	 * 
	 * @param unique
	 *            descript if it is an unique index (primary key) or a multi
	 *            value index is.
	 ...
	 * 
	 * @param digit
	 *            The digit must be in the range 0 between 255.
	 */
	 ...
	 *            The digit must be in the range 0 between 255.
	 * @param rowOffset
	 *            The value that is saved at the end of the tree.
	 */
	final void addNode(char digit, long rowOffset) throws SQLException {
		IndexNode node = addNode(digit);
		if (node.remainderKey != null)
			node.moveRemainderValue();
		node.saveValue(rowOffset);
	}

	/**
	 * Save the rowOffset on the digit position. This can be used for
	 * FINAL_VALUE or REMAINDER_VALUE. The caller need to verify that there
	 * already exist an equals value. This means that the digit and the
	 * remainder is equals.
	 * 
	 * @param digit
	 *            The digit must be in the range 0 between 255.
	 * @param rowOffset
	 *            The value that is saved in the tree.
	 */
	final void saveValue(long rowOffset) throws SQLException {
		if (unique) {
			if (value != null)
				throw SmallSQLException.create(Language.KEY_DUPLICATE);
	...
	/**
	 * Add a value on a tree node end without roll out the completly tree. This
	 * reduce the size of the tree if there are large enties with a high
	 * significance. for example: If you have large strings which are different
	 * on the on the first 3 charchters. Then you need only a tree size of 3.
	 * 
	 * @param digit
	 *            The digit must be in the range 0 between 255.
	 * @param rowOffset
	 *            The result value. This is the value that is saved in the tree.
	 * @param value
	 *            The key value.
	 * @param digitCount
	 *            The count of digits from value that need to indexing in the
	 *            tree. The range is from 1 to 3;
	 */
	final void addRemainderKey(long rowOffset, long remainderValue,
			int charCount) throws SQLException {
		saveRemainderValue(remainderValue, charCount);
		value = (unique) ? (Object) new Long(rowOffset) : new LongTreeList(
				rowOffset);
	}

	final void addRemainderKey(long rowOffset, char[] remainderValue, int offset)
			throws SQLException {
	...
	final IndexNode addRootValue(char[] remainderValue, int offset)
			throws SQLException {
		saveRemainderValue(remainderValue, offset);
		return addRoot();
	}

	final IndexNode addRootValue(long remainderValue, int digitCount)
			throws SQLException {
	...
	private final void moveRemainderValueSub(Object rowOffset,
			char[] remainderValue) {
	...
	/**
	 * Return the root IndexNode for the digit. If does not exists then it
	 * create one.
	 * 
	 * @param digit
	 */
	...
	void save(StoreImpl output) throws SQLException {
		...
		if (length > 0)
			output.writeChars(remainderKey);
		...
		} else if (value instanceof LongTreeList) {
			output.writeByte(2);
			((LongTreeList) value).save(output);
		} else if (value instanceof IndexNode) {
			output.writeByte(3);
	...
	void load(StoreImpl input) throws SQLException {
		...
		case 3:
			...
			break;
		default:
			throw SmallSQLException.create(Language.INDEX_CORRUPT,
					String.valueOf(valueType));
		}
