...
public class ExpressionArithmetic extends Expression {
	 ...
	 */
	ExpressionArithmetic(Expression left, Expression right, Expression right2,
			int operation) {
		super(FUNCTION);
		this.left = left;
		this.right = right;
	...
	ExpressionArithmetic(Expression left, Expressions inList, int operation) {
		...
		if (inList != null) {
			...
			// Occur with ExpressionInSelect, in this case the method isInList()
			// is overridden
	...
	private Expression convertExpressionIfNeeded(Expression expr,
			Expression other) {
		...
		case SQLTokenizer.BINARY:
			...
			case SQLTokenizer.BINARY:
				if (other.getPrecision() > expr.getPrecision()) {
					return new ExpressionFunctionConvert(new ColumnExpression(
							other), expr, null);
				}
				break;
			}
	...
	public boolean equals(Object expr) {
		...
		if (!(expr instanceof ExpressionArithmetic))
			return false;
		if (((ExpressionArithmetic) expr).operation != operation)
			return false;
		return true;
	}
	...
	private double getDoubleImpl(double lVal, double rVal)
	...
	private MutableNumeric getNumericImpl() throws java.lang.Exception {
		...
		default:
			throw createUnspportedConversion(SQLTokenizer.NUMERIC);
		}
	}
	...
	Object getObject() throws java.lang.Exception {
		...
		case SQLTokenizer.JAVA_OBJECT:
			...
			if (lObj instanceof Number && rObj instanceof Number)
				return new Double(getDoubleImpl(((Number) lObj).doubleValue(),
						((Number) rObj).doubleValue()));
			else
	...
	boolean getBoolean() throws java.lang.Exception {
		...
		case LIKE:
			return Utils.like(left.getString(), right.getString());
		case ISNULL:
			return left.isNull();
		case ISNOTNULL:
			return !left.isNull();
		...
		case SQLTokenizer.DOUBLE:
			...
			case BETWEEN:
				...
				return _left >= right.getDouble()
						&& right2.getDouble() >= _left;
			default:
				return getDouble() != 0;
			}
		case SQLTokenizer.MONEY:
		...
		case SQLTokenizer.CLOB: {
			...
			case BETWEEN:
				return comp >= 0
						&& 0 >= String.CASE_INSENSITIVE_ORDER.compare(leftStr,
								right2.getString());
			case ADD:
				return Utils.string2boolean(leftStr + rightStr);
			}
			break;
		}
		case SQLTokenizer.BINARY:
		...
		case SQLTokenizer.UNIQUEIDENTIFIER: {
			...
			case BETWEEN:
				return comp >= 0
						&& 0 >= Utils
								.compareBytes(leftBytes, right2.getBytes());
			}
			break;
		}
		}
	...
	final private String getString(String lVal, String rVal)
			throws java.lang.Exception {
		switch (operation) {
		case ADD:
			return lVal + rVal;
	...
	boolean isNull() throws Exception {
		...
		case ISNOTNULL:
		case IN:
			return false; // Boolean operations return ever a result ???, but at
							// least ISNULL and ISNOTNULL
		case NEGATIVE:
		case BIT_NOT:
			return left.isNull();
		default:
	...
	SQLException createUnspportedDataType() {
		Object[] params = { SQLTokenizer.getKeyWord(getDataType(left, right)),
				getKeywordFromOperation(operation) };
		return SmallSQLException.create(Language.UNSUPPORTED_DATATYPE_OPER,
				params);
	}

	SQLException createUnspportedConversion(int dataType) {
		int type = left == null ? right.getDataType()
				: getDataType(left, right);
		Object[] params = new Object[] { SQLTokenizer.getKeyWord(dataType),
				SQLTokenizer.getKeyWord(type),
				getKeywordFromOperation(operation) };
		return SmallSQLException.create(Language.UNSUPPORTED_CONVERSION_OPER,
				params);
	}
	...
	/**
	 * This method only for creating an error message. Thats there is no
	 * optimizing.
	 * 
	 * @param value
	 * @return
	...
	/**
	 * Return the best data type for a complex number operation. This method
	 * return only SQLTokenizer.INT, SQLTokenizer.BIGINT, SQLTokenizer.MONEY,
	 * SQLTokenizer.DECIMAL or SQLTokenizer.DOUBLE.
	 * 
	 * @param paramDataType
	 */
	...
	static int getDataType(int typeLeft, int typeRight) {
		...
		int dataTypeIdx = Math.min(Utils.indexOf(typeLeft, DatatypeRange),
				Utils.indexOf(typeRight, DatatypeRange));
		if (dataTypeIdx < 0)
	...
	private static final int[] DatatypeRange = { SQLTokenizer.TIMESTAMP,
			SQLTokenizer.SMALLDATETIME, SQLTokenizer.DATE, SQLTokenizer.TIME,
			SQLTokenizer.DOUBLE, SQLTokenizer.FLOAT, SQLTokenizer.REAL,
			SQLTokenizer.DECIMAL, SQLTokenizer.NUMERIC, SQLTokenizer.MONEY,
			SQLTokenizer.SMALLMONEY, SQLTokenizer.BIGINT, SQLTokenizer.INT,
			SQLTokenizer.SMALLINT, SQLTokenizer.TINYINT, SQLTokenizer.BIT,
			SQLTokenizer.BOOLEAN, SQLTokenizer.LONGNVARCHAR,
			SQLTokenizer.UNIQUEIDENTIFIER, SQLTokenizer.NVARCHAR,
			SQLTokenizer.NCHAR, SQLTokenizer.VARCHAR, SQLTokenizer.CHAR,
			SQLTokenizer.LONGVARCHAR, SQLTokenizer.CLOB,
			SQLTokenizer.VARBINARY, SQLTokenizer.BINARY,
			SQLTokenizer.LONGVARBINARY, SQLTokenizer.BLOB, SQLTokenizer.NULL };

	private static int NVARCHAR_IDX = Utils.indexOf(SQLTokenizer.NVARCHAR,
			DatatypeRange);
	private static int INT_IDX = Utils.indexOf(SQLTokenizer.INT, DatatypeRange);
	private static int BIGINT_IDX = Utils.indexOf(SQLTokenizer.BIGINT,
			DatatypeRange);
	private static int MONEY_IDX = Utils.indexOf(SQLTokenizer.MONEY,
			DatatypeRange);
	private static int DECIMAL_IDX = Utils.indexOf(SQLTokenizer.DECIMAL,
			DatatypeRange);
}