...
class Utils {
	...
	static byte[] hex2bytes(char[] hex, int offset, int length)
			throws SQLException {
		try {
			...
			throw SmallSQLException.create(Language.SEQUENCE_HEX_INVALID,
	...
	private static byte hex2byte(char[] hex, int offset) throws SQLException {
		try {
			...
			throw SmallSQLException.create(Language.SEQUENCE_HEX_INVALID_STR,
	 ...
	 * @throws SQLException
	 */
	static CommandSelect createMemoryCommandSelect(SSConnection con,
			String[] colNames, Object[][] data) throws SQLException {
	 ...
	 */
	static final FileChannel openRaFile(File file, boolean readonly)
			throws FileNotFoundException, SQLException {

		RandomAccessFile raFile = new RandomAccessFile(file, readonly ? "r"
				: "rw");
		...
		if (!readonly) {
			try {
				...
				if (lock == null) {
					throw SmallSQLException.create(Language.CANT_LOCK_FILE,
							file);
				}
				...
				throw SmallSQLException.createFromException(
						Language.CANT_LOCK_FILE, file, th);
			}
		}
		return channel;
	}
	 ...
	 */
	private static final void getExpressionNameFromTree(Expressions list,
			Expression tree) {
