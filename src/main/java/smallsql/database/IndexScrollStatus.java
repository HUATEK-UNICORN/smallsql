...
/**
 * This class save the status of the scrolling through a Index. This is like a
 * Cursor.
 */
class IndexScrollStatus {
	private final IndexNode rootPage;
	private final Expressions expressions; // is used for the description of ASC
											// and DESC

	private final Stack<IndexNodeScrollStatus> nodeStack = new Stack<IndexNodeScrollStatus>(); // TODO
																		// performance
																		// Stack
																		// durch
																		// nicht
	...
	/**
	 * Return the next rowOffset of this index. You need to call reset() before
	 * the first use.
	 * 
	 * @param next
	 *            if true the next rowOffset else the previous rowOffset
	 */
	final long getRowOffset(boolean scroll) {
		if (longList != null) {
			long rowOffset = scroll ? longList.getNext(longListEnum) : longList
					.getPrevious(longListEnum);
			if (rowOffset < 0) {
		...
		while (true) {
			IndexNodeScrollStatus status = nodeStack
					.peek();
			...
			if (!status.asc ^ scroll) {
				...
				if (idx == -1) {
					if (status.nodeValue != null) {
						if (status.nodeValue instanceof IndexNode) {
							level++;
							nodeStack
									.push(new IndexNodeScrollStatus(
											(IndexNode) status.nodeValue,
				...
				if (idx >= status.nodes.length) {
					...
					if (nodeStack.size() > 1) {
						...
						status.idx = status.nodes.length; // to prevent problems
															// with scroll back
															// after multiple
															// calls after the
															// end.
				...
				nodeStack.push(new IndexNodeScrollStatus(node, status.asc,
						scroll, level));
			} else {
				// DESC order
				int idx = --status.idx;
	...
	/**
	 * Move the index after the last position. The next call nextRowOffset()
	 * returns a -1
	 * 
	 */
	 ...
	 */
	/*
	 * final boolean isAfterLast(){ if(longList != null || nodeStack.size() !=
	 * 1){ return false; } return
	 * ((IndexNodeScrollStatus)nodeStack.peek()).isAfterLast(); }
	 */
	...
	private final long getReturnValue(Object value) {
		if (rootPage.getUnique()) {
			...
			longListEnum.reset();
			return longList.getNext(longListEnum); // there be should one value
													// as minimum
		}
