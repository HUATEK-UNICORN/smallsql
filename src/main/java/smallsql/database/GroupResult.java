...
class GroupResult extends MemoryResult {
	...
	private Expression currentGroup; // Validate if the current row of cmd is
										// part of the current group
	private RowSource from;
	private Expressions groupBy; // the list of Expressions in the GROUP BY
									// clause
	private Expressions expressions = new Expressions(); // List of Expression
	private Expressions internalExpressions = new Expressions(); // a list of
																	// Aggregate
																	// Function
																	// and
																	// ColNames
	 ...
	 */
	GroupResult(CommandSelect cmd, RowSource from, Expressions groupBy,
			Expression having, Expressions orderBy) throws SQLException {
		this.from = from;
		...
		if (groupBy != null) {
			for (int i = 0; i < groupBy.size(); i++) {
				...
				Expression expr = new ExpressionArithmetic(left, right,
						ExpressionArithmetic.EQUALS_NULL);
				currentGroup = (currentGroup == null) ? expr
						: new ExpressionArithmetic(currentGroup, expr,
								ExpressionArithmetic.AND);
			}
		}
		...
		if (having != null)
			having = patchExpression(having);
		patchExpressions(orderBy);
	}

	/**
	 * Add a expression to the internal expression list if not exist in this
	 * list. It will be added named columns in the GROUP BY clause.
	 * 
	 * @param expr
	 *            The expression to added.
	 * @return the position in the internal list
	 */
	final private int addInternalExpressionFromGroupBy(Expression expr)
			throws SQLException {
	...
	/**
	 * Add a expression to the internal expression list if not exist in this
	 * list. It will be added aggregate functions from the SELECT, HAVING and
	 * ORDER BY clause.
	 * 
	 * @param expr
	 *            The expression to added.
	 * @return the position in the internal list
	 */
	final private int addInternalExpressionFromSelect(Expression expr)
			throws SQLException {
		int type = expr.getType();
		if (type == Expression.NAME) {
		...
		} else if (type >= Expression.GROUP_BEGIN) {
			int idx = internalExpressions.indexOf(expr);
			if (idx >= 0)
				return idx;
			internalExpressions.add(expr);
			...
			// if a function or arithmetic expression is already in the group by
			// the it is OK
			int idx = internalExpressions.indexOf(expr);
			if (idx >= 0)
				return idx;
	...
	/**
	 * Patch all external ExpressionName in the list (SELECT clause) that it
	 * link to the the internal RowSource.
	 */
	...
	final private void patchExpressions(Expression expression)
			throws SQLException {
	 ...
	 * 
	 * @param expr
	 *            the Expression to patch
	 * @return on simple columns and Aggregatfunction the original Expression is
	 *         return as patch.
	 */
	final private Expression patchExpression(Expression expr)
			throws SQLException {
		...
		if (idx >= 0) {
			...
			if (expr instanceof ExpressionName) {
				...
				// this can only occur if in the GROUP BY clause are a function
	...
	/**
	 * Add a new Row to the MemoryResult. This occur because the GROUP BY clause
	 * of the current row not relate to an exists row.
	 * 
	 */
	final private void addGroupRow() {
		// add a new row to the GroupResult
		ExpressionValue[] newRow = currentRow = new ExpressionValue[expressions
				.size()];
		for (int i = 0; i < newRow.length; i++) {
			Expression expr = expressions.get(i);
			int type = expr.getType();
			if (type < Expression.GROUP_BEGIN)
				type = Expression.GROUP_BY;
