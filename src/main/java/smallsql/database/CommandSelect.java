...
class CommandSelect extends Command {
				...
				if (tableAlias != null) {
					...
					for (; t < tables.size(); t++) {
						...
						if (tableAlias.equalsIgnoreCase(fromEntry.getAlias())) {
							...
							i = compileAdd_All_Table_Columns(fromEntry, table,
									i) - 1;
							break;
						}
					}
	 ...
	 * table source.
	 */
	private void compileLinkExpressionName(ExpressionName expr)
			throws Exception {
		String tableAlias = expr.getTableAlias();
		if (tableAlias != null) {
			int t = 0;
			for (; t < tables.size(); t++) {
				...
				if (tableAlias.equalsIgnoreCase(fromEntry.getAlias())) {
					...
					if (colIdx >= 0) {
						...
						throw SmallSQLException.create(
								Language.COL_INVALID_NAME,
								new Object[] { expr.getName() });
				}
			}
			if (t == tables.size())
				throw SmallSQLException.create(Language.COL_WRONG_PREFIX,
						tableAlias);
		} else {
			...
			for (int t = 0; t < tables.size(); t++) {
				...
				if (colIdx >= 0) {
					if (isSetFrom) {
						...
						// ambiguous
						throw SmallSQLException.create(Language.COL_AMBIGUOUS,
								expr.getName());
					}
					// Column was find and now we set the DataSouce, column
					// index and TableView.
			...
			if (!isSetFrom) {
				throw SmallSQLException.create(Language.COL_INVALID_NAME,
						expr.getName());
			}
		}
		compileLinkExpressionParams(expr);
	...
	private final int compileAdd_All_Table_Columns(DataSource fromEntry,
			TableView table, int position) {
		for (int k = 0; k < table.columns.size(); k++) {
			ExpressionName expr = new ExpressionName(table.columns.get(k)
					.getName());
	...
	final void updateRow(SSConnection con, Expression[] newRowSources)
			throws SQLException {
		int savepoint = con.getSavepoint();
		try {
			// loop through all tables of this ResultSet
			for (int t = 0; t < tables.size(); t++) {
				TableViewResult result = TableViewResult
						.getTableViewResult(tables.get(t));
				...
				for (int i = 0; i < columnExpressions.size(); i++) {
					...
					if (src != null
							&& (!(src instanceof ExpressionValue) || !((ExpressionValue) src)
									.isEmpty())) {
						Expression col = columnExpressions.get(i);
						if (!col.isDefinitelyWritable())
							throw SmallSQLException.create(
									Language.COL_READONLY, new Integer(i));
	...
	final void insertRow(SSConnection con, Expression[] newRowSources)
			throws SQLException {
		if (tables.size() > 1)
		...
		int savepoint = con.getSavepoint();
		try {
			TableViewResult result = TableViewResult.getTableViewResult(tables
					.get(0));
			TableView table = result.getTableView();
