...
abstract class TableView {
	...
	static final int LOCK_READ = 2; // occur on read and prevent a write of
									// data, it can occur more as one LOCK_READ
									// per page
	static final int LOCK_WRITE = 3; // occur on write and prevent every other
										// access to the data, it is only one
										// LOCK_WRITE per page possible
	 ...
	 */
	static TableView load(SSConnection con, Database database, String name)
			throws SQLException {
		...
		try {
			...
			if (!file.exists())
				throw SmallSQLException.create(Language.TABLE_OR_VIEW_MISSING,
						name);
			...
			default:
				throw SmallSQLException.create(
						Language.TABLE_OR_VIEW_FILE_INVALID, fileName);
			}
			if (version > TABLE_VIEW_VERSION)
				throw SmallSQLException.create(Language.FILE_TOONEW,
						new Object[] { new Integer(version), fileName });
			if (version < TABLE_VIEW_OLD_VERSION)
				throw SmallSQLException.create(Language.FILE_TOOOLD,
						new Object[] { new Integer(version), fileName });
			if (magic == MAGIC_TABLE)
				return new Table(database, con, name, raFile,
						raFile.position(), version);
	 ...
	 *             SecurityException
	 */
	FileChannel createFile(SSConnection con, Database database)
			throws Exception {
