...
final class TableResult extends TableViewResult {
	...
	/**
	 * A List of rows that was inserted. The rows can be uncommited in memory or
	 * commited on harddisk. The WHERE condition does not need to be valid.
	 */
	private List<StorePageLink> insertStorePages;
	/**
	 * The filePos of the first own insert with insertRow(). On this rows the
	 * WHERE condition is not verify.
	 */
	private long firstOwnInsert;
	/**
	 * The max fileOffset at open the ResultSet. Rows that are commited later
	 * are not not counted.
	 */
	...
	/**
	 * Move to the row in the filePos. A value of -1 for filePos is invalid at
	 * this call point.
	 */
	final private boolean moveToRow() throws Exception {
		if (filePos >= 0x4000000000000000L) {
			store = (insertStorePages
					.get((int) (filePos & 0x3FFFFFFFFFFFFFFFL))).getStore(
					table, con, lock);
		} else {
			store = (filePos < maxFileOffset) ? table.getStore(con, filePos,
					lock) : null;
			if (store == null) {
				if (insertStorePages.size() > 0) {
					filePos = 0x4000000000000000L;
					store = (insertStorePages
							.get((int) (filePos & 0x3FFFFFFFFFFFFFFFL)))
							.getStore(table, con, lock);
				}
			}
		}
		if (store != null) {
	...
	/**
	 * Move to the next valid row. A valid row is a normal row or an pointer to
	 * an updated row value. A invalid row is a deleted row or an updated value
	 * that is reference by an update pointer.
	 */
	...
	/**
	 * A negative filePos means no more rows.
	 * <p>
	 * A value larger 0x4000000000000000L means a row that was inserted in this
	 * ResultSet.
	 * <p>
	 * A value of 0 means beforeFirst.
	 * <p>
	 * All other values are read position in the file.
	 * <p>
	 * 
	 */
	...
	final boolean rowDeleted() {
		...
		if (store instanceof StoreImpl && ((StoreImpl) store).isRollback()) {
			return true;
		}
		return false;
	}
	...
	final private void copyValueInto(int colIdx, StoreImpl dst) {
		...
		int length = (colIdx < offsets.length ? offsets[colIdx] : store
				.getUsedSize()) - offset;
