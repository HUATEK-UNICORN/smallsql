...
class StorePageMap {
	...
	/**
	 * Returns <tt>true</tt> if this map contains a StorePage for the specified
	 * key.
	 * 
	 */
	...
	/**
	 * Add the StorePage with the key. Multiple StorePage for the same key are
	 * valid. The cause are multiple changes in one transaction. With SavePoints
	 * a rollback to a older StorePage is valid.
	 * <p>
		...
		for (int j = 0; j < src.length; j++) {
			...
			if (e != null) {
				...
				do {
					...
					// The order for StorePages with the same key must not
					// change
					// that we need to find the end of the link list. This is
					// different to a typical HashTable
					if (newTable[i] == null) {
						newTable[i] = e;
					} else {
