...
public class CommandInsert extends Command {
	...
	boolean noColumns; // after the table name was no columnExpressions; all
						// columnExpressions in default order
		...
		if (noColumns) {
			...
			for (int c = 0; c < columnExpressions.size(); c++) {
				...
				if (idx >= 0) {
					...
					throw SmallSQLException.create(Language.COL_MISSING,
							sqlColName);
				}
			}
		...
		while (cmdSel.next()) {
			...
			for (int c = 0; c < matrix.length; c++) {
				...
				if (idx >= 0) {
					...
					if (needGeneratedKeys && generatedKeysType == 0
							&& valueExpress != Expression.NULL) {
			...
			if (needGeneratedKeys) {
				...
				keys.toArray(data[0]);
				st.setGeneratedKeys(new SSResultSet(st, Utils
						.createMemoryCommandSelect(con,
								keyColumnNames.toArray(), data)));
			}
		}
	}
