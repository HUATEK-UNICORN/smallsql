...
class Table extends TableView {
	...
	final private Map<Long, TableStorePage> locks = new HashMap<Long, TableStorePage>();
	private SSConnection tabLockConnection; // if set then it is the Connection
											// with a LOCK_TAB
	private int tabLockCount;
	/** if set then it is the Connection with a LOCK_WRITE_TAB */
	final private List<TableStorePage> locksInsert = new ArrayList<TableStorePage>(); // liste
																						// der
	// LOCK_INSERT
	final private Map<SSConnection, SSConnection> serializeConnections = new HashMap<SSConnection, SSConnection>();
	 ...
	 */
	Table(Database database, SSConnection con, String name, FileChannel raFile,
			long offset, int tableFormatVersion) throws Exception {
		...
		if (store == null) {
			throw SmallSQLException.create(Language.TABLE_FILE_INVALID,
					getFile(database));
		}
	 ...
	 */
	Table(Database database, SSConnection con, String name, Columns columns,
			IndexDescriptions indexes, ForeignKeys foreignKeys)
			throws Exception {
	 ...
	 */
	Table(Database database, SSConnection con, String name, Columns columns,
			IndexDescriptions existIndexes, IndexDescriptions newIndexes,
		...
		for (int i = 0; i < foreignKeys.size(); i++) {
			...
			Table pkTable = (Table) database.getTableView(con,
					foreignKey.pkTable);
	...
	/**
	 * Drop the Table. This method is static that the file does not need to load
	 * and also corrupt files can be dropped.
	 */
	static void drop(Database database, String name) throws Exception {
		boolean ok = new File(Utils.createTableViewFileName(database, name))
				.delete();
		if (!ok)
			throw SmallSQLException.create(Language.TABLE_CANT_DROP, name);
	}
	...
	void drop(SSConnection con) throws Exception {
		...
		if (storePage == null) {
			throw SmallSQLException.create(Language.TABLE_CANT_DROP_LOCKED,
					name);
		}
	...
	private void write(SSConnection con) throws Exception {
		...
		for (int i = 0; i < indexes.size(); i++) {
			...
			store.setCurrentOffsetInPage(offsetStart + 4); // place holder for
															// length
		...
		store.writeFinsh(null); // The connection parameter is null because the
								// table header is written immediately.
		firstPage = store.getNextPagePos();
	}
	...
	/*
	 * StoreImpl getStoreCreate( SSConnection con, long filePos ) throws
	 * Exception{ return StoreImpl.createStore( con, raFile,
	 * SQLTokenizer.CREATE, filePos ); }
	 */

	StoreImpl getStore(SSConnection con, long filePos, int pageOperation)
			throws Exception {
	...
	StoreImpl getStore(TableStorePage storePage, int pageOperation)
			throws Exception {
	...
	/*
	 * StoreImpl getStoreUpdate( SSConnection con, long filePos ) throws
	 * Exception{ return StoreImpl.createStore( con, raFile,
	 * SQLTokenizer.UPDATE, filePos ); }
	 * 
	 * StoreImpl getStoreDelete( SSConnection con, long filePos ) throws
	 * Exception{ return StoreImpl.createStore( con, raFile,
	 * SQLTokenizer.DELETE, filePos ); }
	 */
	...
	StoreImpl getLobStore(SSConnection con, long filePos, int pageOperation)
			throws Exception {
		if (lobs == null) {
			lobs = new Lobs(this);
		}
	...
	/**
	 * Return the file offset of the first page with data after the table
	 * declaration. This is equals to the first row.
	 */
	...
	/**
	 * Return a list of Links to not commited rows. The list include only the
	 * rows that are visible for the current isolation level.
	 */
	List<StorePageLink> getInserts(SSConnection con) {
		synchronized (locks) {
			List<StorePageLink> inserts = new ArrayList<StorePageLink>();
			if (con.isolationLevel <= Connection.TRANSACTION_READ_UNCOMMITTED) {
				for (int i = 0; i < locksInsert.size(); i++) {
					TableStorePageInsert lock = (TableStorePageInsert) locksInsert
							.get(i);
	...
	/**
	 * Request a page lock. If the request is valid then it return the
	 * StorePage. If the lock can not be created within 5 seconds then it throw
	 * an exception.
	 * 
	 * @param con
	 *            The connection that request the lock
	 * @param pageOperation
	 *            The operation that should be perform
	 * @param page
	 ...
	 *             if a timeout occurs
	 */
	final TableStorePage requestLock(SSConnection con, int pageOperation,
		...
		synchronized (locks) {
			...
			long endTime = 0;
			while (true) {
				TableStorePage storePage = requestLockImpl(con, pageOperation,
						page);
				if (storePage != null)
				...
				if (waitTime <= 0)
					throw SmallSQLException.create(Language.TABLE_DEADLOCK,
							name);
				locks.wait(waitTime);
			}
		}
	...
	/**
	 * Request a page lock. If the request is valid then it return the
	 * StorePage. In the other case it return null.
	 * 
	 * @param page
	 *            The fileOffset or -1 for a new page
	 * @throws SQLException
	 */
	final private TableStorePage requestLockImpl(SSConnection con,
			int pageOperation, long page) throws SQLException {
		synchronized (locks) {
			if (tabLockConnection != null && tabLockConnection != con)
				return null;
			...
			case SQLTokenizer.CREATE: {
				// first check if another connection has a lock before creating
				// a table lock
				if (locks.size() > 0) {
					Iterator<TableStorePage> values = locks.values().iterator();
				...
				for (int i = 0; i < locksInsert.size(); i++) {
					// the first StorePage in the linked list must be ever
					// TableStorePageInsert
					TableStorePageInsert lock = (TableStorePageInsert) locksInsert
							.get(i);
					if (lock.con != con)
						return null;
				...
				tabLockCount++;
				TableStorePage lock = new TableStorePage(con, this, LOCK_TAB,
						page);
				con.add(lock);
				return lock;
			}
			...
			case SQLTokenizer.INSERT: {
				// if there are more as one Connection with a serializable lock
				// then an INSERT is not valid
				if (serializeConnections.size() > 1)
					return null;
				if (serializeConnections.size() == 1
						&& serializeConnections.get(con) == null)
					return null;
				TableStorePageInsert lock = new TableStorePageInsert(con, this,
						LOCK_INSERT);
			...
			case SQLTokenizer.UPDATE: {
				...
				if (con.isolationLevel >= Connection.TRANSACTION_REPEATABLE_READ
					...
					lock.lockType = pageOperation == SQLTokenizer.UPDATE ? LOCK_WRITE
							: LOCK_READ;
					if (prevLock != null) {
			...
			case SQLTokenizer.LONGVARBINARY:
				...
				// the difference to INSERT is that page described the size of
				// the byte buffer
	...
	/**
	 * Request a write lock for a page that is read. It add the resulting
	 * StorePage to the list of commits.
	 * 
	 * @throws SQLException
	 *             if the connection was closed.
	 */
	TableStorePage requestWriteLock(SSConnection con, TableStorePage readlock)
			throws SQLException {
		...
		while (lock != null) {
			if (lock.con != con)
				return null; // there is already any lock from another
								// connection, we can not start write
			if (lock.lockType < LOCK_WRITE) {
		...
		synchronized (locks) {
			try {
							...
							if (lock == storePage) {
								...
								if (lock == prev) {
									if (lock.nextLock == null) {
										// the first lock is the only lock in
										// the list
										locksInsert.remove(i--);
									} else {
										// only the first lock of the list is
										// remove
				...
				case LOCK_WRITE:
					...
					while (lock != null) {
						if (lock == storePage) {
							...
							if (lock == prev) {
								...
								// a lock in the middle or end of the list is
								// removed
								prev.nextLock = lock.nextLock;
							}
							return;
					...
					// a run through can occur if a lock was step high and the
					// type does not compare
					break;
				case LOCK_TAB:
					assert storePage.con == tabLockConnection : "Internal Error with TabLock";
					if (--tabLockCount == 0)
						tabLockConnection = null;
					break;
				default:
					throw new Error();
				}
