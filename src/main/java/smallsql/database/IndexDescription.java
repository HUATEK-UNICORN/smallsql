...
final class IndexDescription {
	 ...
	 *            one of SQLTokenizer.PRIMARY, SQLTokenizer.UNIQUE,
	 *            SQLTokenizer.FOREIGN or SQLTokenizer.INDEX.
	 * @param columns
	 *            the Expressions that build the index. For example one or more
	 *            database columns.
	 */
	IndexDescription(String name, String tableName, int constraintType,
			Expressions expressions, Strings columns) {
	...
	final boolean isUnique() {
		return constraintType == SQLTokenizer.PRIMARY
				|| constraintType == SQLTokenizer.UNIQUE;
	}
	 ...
	 * 
	 * @param strings
	 *            a list of columns that should match
	...
	final int matchFactor(Strings strings) {
		...
		nextColumn: for (int c = 0; c < columns.size(); c++) {
	...
	/**
	 * Create a binding of the columns form this index to the columns of the
	 * table.
	 * 
	 * @param database
	 * @param tableView
	 ...
	 */
	final void create(SSConnection con, Database database, TableView tableView)
			throws Exception {
	...
	private FileChannel createFile(SSConnection con, Database database)
			throws Exception {
		...
		if (!ok)
			throw SmallSQLException.create(Language.INDEX_EXISTS, name);
	...
	private void load(Database database) throws SQLException {
		try {
			...
			if (magic != MAGIC_INDEX) {
				throw SmallSQLException.create(Language.INDEX_FILE_INVALID,
						file.getName());
			}
	 ...
	 *            the new value of the current row.
	 */
	 ...
	 * 
	 * @param con
	 *            the connection for a later commit or rollback.
	 */
	 ...
	 */
	final static IndexDescription load(Database database, TableView tableView,
			StoreImpl store) throws SQLException {
		...
		IndexDescription indexDesc = new IndexDescription(store.readString(),
				tableView.name, constraintType, expressions, columns);
