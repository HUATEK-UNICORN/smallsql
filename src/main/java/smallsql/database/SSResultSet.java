...
public class SSResultSet implements ResultSet {
	...
	public String getString(int columnIndex) throws SQLException {
		try {
			...
			if (obj instanceof byte[]) {
				// The Display Value of a binary Value is different as the
				// default in SQL
	...
	public boolean first() throws SQLException {
		try {
			if (st.rsType == ResultSet.TYPE_FORWARD_ONLY)
	...
	public int getType() throws SQLException {
		return getCmd().from.isScrollable() ? ResultSet.TYPE_SCROLL_SENSITIVE
				: ResultSet.TYPE_FORWARD_ONLY;
	}

	@Override
	public int getConcurrency() {
		return isUpdatable ? ResultSet.CONCUR_UPDATABLE
	...
	public void updateBoolean(int columnIndex, boolean x) throws SQLException {
		updateValue(columnIndex, x ? Boolean.TRUE : Boolean.FALSE,
	...
	public Date getDate(int columnIndex, Calendar cal) throws SQLException {
		try {
			...
			if (wasNull)
				return null;
			return new Date(DateTime.addDateTimeOffset(expr.getLong(),
					cal.getTimeZone()));
		} catch (Exception e) {
			throw SmallSQLException.createFromException(e);
		}
	...
	public Time getTime(int columnIndex, Calendar cal) throws SQLException {
		try {
			...
			if (wasNull)
				return null;
			return new Time(DateTime.addDateTimeOffset(expr.getLong(),
					cal.getTimeZone()));
		} catch (Exception e) {
			throw SmallSQLException.createFromException(e);
		}
	...
	public Timestamp getTimestamp(int columnIndex, Calendar cal)
		...
		try {
			...
			if (wasNull)
				return null;
			return new Timestamp(DateTime.addDateTimeOffset(expr.getLong(),
					cal.getTimeZone()));
		} catch (Exception e) {
			throw SmallSQLException.createFromException(e);
		}
	...
	/**
	 * Get the expression of a column. This expression can be used to request a
	...
	final private ExpressionValue getUpdateValue(int columnIndex)
			throws SQLException {
		if (values == null) {
	...
	final private void updateValue(int columnIndex, Object x, int dataType)
			throws SQLException {
		getUpdateValue(columnIndex).set(x, dataType);
		if (st.con.log.isLogging()) {

			st.con.log.println("parameter '"
					+ metaData.getColumnName(columnIndex) + "' = " + x
					+ "; type=" + dataType);
		}
	}
