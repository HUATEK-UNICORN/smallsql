...
class CommandUpdate extends CommandSelect {
	 ...
	 * "UPDATE table1 SET col1 = 234" "col1" is the dest "234" is the source
	 * 
	 * @param dest
	 * @param source
	 *            must be a ExpressionName
	 */
	 ...
	 * if(super.compile(con)){ TableView table =
	 * TableViewResult.getTableViewResult(join).getTableView(); newRowSources =
	 * new Expression[table.columns.size()]; for(int i=0; i<destinations.size();
	 * i++){ String name = destinations.get(i).getName(); int colIdx =
	 * table.findColumn( name ); if(colIdx>=0){ // Column found
	 * newRowSources[colIdx] = sources.get(i); break; }else throw
	 * SmallSQLException.createSQLException(Language.Invalid column name '" +
	...
	void executeImpl(SSConnection con, SSStatement st) throws Exception {
		...
		// this is needed for the case that a writing value depends on reading
		// value
