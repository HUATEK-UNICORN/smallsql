...
public class SSDriver implements Driver {
	...
	static {
		try {
			...
			// DriverManager.setLogStream(new java.io.PrintStream(new
	...
	private Properties parse(String url, Properties info) throws SQLException {
		...
		if (idx1 > 0) {
			String dbPath = (idx2 > 0) ? url.substring(idx1 + 1, idx2) : url
					.substring(idx1 + 1);
		...
		if (idx2 > 0) {
			...
			while (tok.hasMoreTokens()) {
				...
				if (keyValue.length() > 0) {
					...
					if (idx1 > 0) {
						String key = keyValue.substring(0, idx1).toLowerCase()
								.trim();
						String value = keyValue.substring(idx1 + 1).trim();
						props.put(key, value);
					} else {
						throw SmallSQLException.create(Language.CUSTOM_MESSAGE,
								"Missing equal in property:" + keyValue);
					}
				}
			}
		}
	...
	public DriverPropertyInfo[] getPropertyInfo(String url, Properties info)
		...
		driverInfos[0] = new DriverPropertyInfo("dbpath",
				props.getProperty("dbpath"));
		return driverInfos;
	}
