...
final class Database {
	...
	private final Map<Connection, Connection> connections = new WeakHashMap<Connection, Connection>();
	 ...
	 */
	static Database getDatabase(String name, SSConnection con, boolean create)
			throws SQLException {
		if (name == null) {
			return null;
		}
		...
		synchronized (databases) {
			Database db = databases.get(dbKey);
			if (db == null) {
				if (create && !file.isDirectory()) {
					CommandCreateDatabase command = new CommandCreateDatabase(
							con.log, name);
	...
	private static Database getDatabase(SSConnection con, String name)
			throws SQLException {
		return name == null ? con.getDatabase(false) : getDatabase(name, con,
				false);
	}
	 ...
	 *             If can't open
	 */
	private Database(String name, File canonicalFile, boolean readonly)
			throws SQLException {
		try {
			this.name = name;
	...
	static final void closeConnection(SSConnection con) throws SQLException {
		synchronized (databases) {
			...
			while (iterator.hasNext()) {
				...
				database.connections.remove(con);
				if (database.connections.size() == 0) {
					try {
						iterator.remove();
						database.close();
					} catch (Exception e) {
	...
	private final void close() throws Exception {
		synchronized (tableViews) {
			Iterator<TableView> iterator = tableViews.values().iterator();
			while (iterator.hasNext()) {
	...
	static TableView getTableView(SSConnection con, String catalog,
	 ...
	 */
	TableView getTableView(SSConnection con, String tableName)
			throws SQLException {
		synchronized (tableViews) {
	...
	static void dropTable(SSConnection con, String catalog, String tableName)
			throws Exception {
	...
	void replaceTable(Table oldTable, Table newTable) throws Exception {
		synchronized (tableViews) {
			...
			if (!oldFile.renameTo(tmpFile)) {
				throw SmallSQLException.create(Language.TABLE_CANT_RENAME,
						oldTable.name);
			}
	...
	static void dropView(SSConnection con, String catalog, String tableName)
			throws Exception {
	...
	void dropView(String viewName) throws Exception {
		synchronized (tableViews) {
			...
			if (view != null && !(view instanceof View))
				throw SmallSQLException.create(Language.VIEWDROP_NOT_VIEW,
						viewName);
	...
	private void checkForeignKeys(SSConnection con, ForeignKeys foreignKeys)
			throws SQLException {
		for (int i = 0; i < foreignKeys.size(); i++) {
			...
			if (!(pkTable instanceof Table)) {
				throw SmallSQLException.create(Language.FK_NOT_TABLE,
						foreignKey.pkTable);
			}
		}
	}
	 ...
	 * @throws Exception
	 */
	void createTable(SSConnection con, String name, Columns columns,
	 ...
	 */
	Table createTable(SSConnection con, String tableName, Columns columns,
			IndexDescriptions oldIndexes, IndexDescriptions newIndexes,
			ForeignKeys foreignKeys) throws Exception {
		checkForeignKeys(con, foreignKeys);
		Table table = new Table(this, con, tableName, columns, oldIndexes,
				newIndexes, foreignKeys);
		synchronized (tableViews) {
	...
	void createView(SSConnection con, String viewName, String sql)
			throws Exception {
		// createFile() can run only one Thread success (it is atomic)
	...
	static Object[][] getCatalogs(Database database) {
		...
		File baseDir = (database != null) ? database.directory.getParentFile()
				: new File(".");
		...
		for (int i = 0; i < dirs.length; i++) {
			...
			if (Utils.like(name, tablePattern)) {
				list.add(name.substring(0, name.length()
						- Utils.TABLE_VIEW_EXTENTION.length()));
			}
		}
		return list;
	}

	Object[][] getColumns(SSConnection con, String tablePattern,
			String colPattern) throws Exception {
		...
		for (int i = 0; i < tables.size(); i++) {
			...
			try {
				...
				for (int c = 0; c < cols.size(); c++) {
					...
					row[4] = Utils.getShort(SQLTokenizer.getSQLDataType(col
					...
					row[10] = Utils
							.getInteger(col.isNullable() ? DatabaseMetaData.columnNullable
	...
	Object[][] getReferenceKeys(SSConnection con, String pkTable, String fkTable)
			throws SQLException {
		List<Object[]> rows = new ArrayList<Object[]>();
		Strings tables = (pkTable != null) ? getTables(pkTable)
				: getTables(fkTable);
		for (int t = 0; t < tables.size(); t++) {
			...
			for (int i = 0; i < references.size(); i++) {
				...
				if ((pkTable == null || pkTable.equals(foreignKey.pkTable))
						&& (fkTable == null || fkTable
								.equals(foreignKey.fkTable))) {
					...
					for (int c = 0; c < columnsPk.size(); c++) {
						...
						row[13] = Utils
								.getShort(DatabaseMetaData.importedKeyNotDeferrable); // DEFERRABILITY
						rows.add(row);
					}
				}
			}
	...
	Object[][] getBestRowIdentifier(SSConnection con, String table)
			throws SQLException {
		List<Object[]> rows = new ArrayList<Object[]>();
			...
			for (int i = 0; i < indexes.size(); i++) {
				...
				if (index.isUnique()) {
					...
					for (int c = 0; c < columns.size(); c++) {
						...
						row[0] = Utils
								.getShort(DatabaseMetaData.bestRowSession);// SCOPE
						...
						row[7] = Utils
								.getShort(DatabaseMetaData.bestRowNotPseudo);// PSEUDO_COLUMN
						rows.add(row);
					}
				}
			}
	...
	Object[][] getPrimaryKeys(SSConnection con, String table)
			throws SQLException {
		List<Object[]> rows = new ArrayList<Object[]>();
		Strings tables = getTables(table);
		for (int t = 0; t < tables.size(); t++) {
	...
	Object[][] getIndexInfo(SSConnection con, String table, boolean unique)
			throws SQLException {
		List<Object[]> rows = new ArrayList<Object[]>();
		Strings tables = getTables(table);
