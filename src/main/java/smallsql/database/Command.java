...
abstract class Command {
	...
	void verifyParams() throws SQLException {
		for (int p = 0; p < params.size(); p++) {
			if (((ExpressionValue) params.get(p)).isEmpty())
				throw SmallSQLException.create(Language.PARAM_EMPTY,
						new Integer(p + 1));
		}
	}
	...
	private ExpressionValue getParam(int idx) throws SQLException {
		if (idx < 1 || idx > params.size())
			throw SmallSQLException
					.create(Language.PARAM_IDX_OUT_RANGE, new Object[] {
							new Integer(idx), new Integer(params.size()) });
	 ...
	 * Set value of a PreparedStatement parameter. The idx starts with 1.
	 */
	void setParamValue(int idx, Object value, int dataType, int length)
			throws SQLException {
	...
	abstract void executeImpl(SSConnection con, SSStatement st)
			throws Exception;
