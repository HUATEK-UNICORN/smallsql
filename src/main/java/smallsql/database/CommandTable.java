...
final class CommandTable extends Command {
	...
	void executeImpl(SSConnection con, SSStatement st) throws Exception {
		Database database = catalog == null ? con.getDatabase(false) : Database
				.getDatabase(catalog, con, false);
		...
		case SQLTokenizer.ADD:
			...
			// Request a TableLock and hold it for the completely ALTER TABLE
			// command
			TableStorePage tableLock = oldTable.requestLock(con,
					SQLTokenizer.ALTER, -1);
			...
			try {
				...
				Table newTable = database.createTable(con, newName, newColumns,
						oldTable.indexes, indexes, foreignKeys);
	...
	private void addColumn(Columns cols, Column column) throws SQLException {
		if (cols.get(column.getName()) != null) {
			throw SmallSQLException.create(Language.COL_DUPLICATE,
					column.getName());
		}
		cols.add(column);
	}
