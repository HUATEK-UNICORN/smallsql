...
public class SSConnection implements Connection {
	...
	int isolationLevel = TRANSACTION_READ_COMMITTED; // see also
	...
	/**
	 * @param returnNull
	 *            If null is a valid return value for the case of not connected
	 *            to a database.
	 * @throws SQLException
	 *             If not connected and returnNull is false.
	 */
		...
		synchronized (getMonitor()) {
			try {
				...
				for (int i = 0; i < count; i++) {
					TransactionStep page = commitPages.get(i);
					page.commit();
				}
				for (int i = 0; i < count; i++) {
					TransactionStep page = commitPages.get(i);
	...
	void rollback(int savepoint) throws SQLException {
		...
		synchronized (getMonitor()) {
			for (int i = commitPages.size() - 1; i >= savepoint; i--) {
				TransactionStep page = commitPages.remove(i);
				page.rollback();
				page.freeLock();
			}
		}
	...
	public void setTransactionIsolation(int level) throws SQLException {
		if (!metadata.supportsTransactionIsolationLevel(level)) {
			throw SmallSQLException.create(Language.ISOLATION_UNKNOWN,
	...
	public PreparedStatement prepareStatement(String sql, int resultSetType,
		...
		return new SSPreparedStatement(this, sql, resultSetType,
	...
	public CallableStatement prepareCall(String sql, int resultSetType,
			int resultSetConcurrency) throws SQLException {
		return new SSCallableStatement(this, sql, resultSetType,
				resultSetConcurrency);
	}
	...
	public void rollback(Savepoint savepoint) throws SQLException {
		...
		throw SmallSQLException.create(Language.SAVEPT_INVALID_DRIVER,
				savepoint);
	}
