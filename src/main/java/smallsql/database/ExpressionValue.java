...
public class ExpressionValue extends Expression {
	...
	/**
	 * Constructor is used from PreparedStatement parameters ( '?' in sql
	 * expression )
	 */
	ExpressionValue() {
		super(VALUE);
	...
	void accumulate(Expression expr) throws Exception {
		...
		case MAX:
			if (value == null) {
				...
				case SQLTokenizer.SMALLINT:
				case SQLTokenizer.INT:
					((MutableInteger) value).value = Math.max(
							((MutableInteger) value).value, expr.getInt());
					break;
				case SQLTokenizer.BIGINT:
					((MutableLong) value).value = Math.max(
							((MutableLong) value).value, expr.getLong());
					break;
				case SQLTokenizer.REAL:
					((MutableFloat) value).value = Math.max(
							((MutableFloat) value).value, expr.getFloat());
					break;
				case SQLTokenizer.FLOAT:
				case SQLTokenizer.DOUBLE:
					((MutableDouble) value).value = Math.max(
							((MutableDouble) value).value, expr.getDouble());
					break;
				...
				case SQLTokenizer.LONGVARCHAR:
					...
					if (String.CASE_INSENSITIVE_ORDER.compare((String) value,
							str) < 0) // cast needed for Compiler 1.5
						value = str;
					break;
				case SQLTokenizer.NUMERIC:
				case SQLTokenizer.DECIMAL:
					...
					break;
				case SQLTokenizer.MONEY:
					((Money) value).value = Math.max(((Money) value).value,
							expr.getMoney());
					break;
				...
				case SQLTokenizer.TIME:
					((DateTime) value).time = Math.max(((DateTime) value).time,
							expr.getLong());
					break;
				case SQLTokenizer.UNIQUEIDENTIFIER:
					// uuid are fixed-len uppercase hex strings and can be
					// correctly
					// compared with compareTo()
					String uuidStr = expr.getString();
					if (uuidStr.compareTo((String) value) > 0)
						value = uuidStr;
					break;
				default:
					String keyword = SQLTokenizer.getKeyWord(dataType);
					throw SmallSQLException.create(
							Language.UNSUPPORTED_TYPE_MAX, keyword);
				}
			}
			break;
		case MIN:
			if (value == null) {
				...
				case SQLTokenizer.SMALLINT:
				case SQLTokenizer.INT:
					((MutableInteger) value).value = Math.min(
							((MutableInteger) value).value, expr.getInt());
					break;
				case SQLTokenizer.BIGINT:
					((MutableLong) value).value = Math.min(
							((MutableLong) value).value, expr.getLong());
					break;
				case SQLTokenizer.REAL:
					((MutableFloat) value).value = Math.min(
							((MutableFloat) value).value, expr.getFloat());
					break;
				case SQLTokenizer.FLOAT:
				case SQLTokenizer.DOUBLE:
					((MutableDouble) value).value = Math.min(
							((MutableDouble) value).value, expr.getDouble());
					break;
				case SQLTokenizer.CHAR:
				case SQLTokenizer.VARCHAR:
				...
				case SQLTokenizer.DECIMAL:
					...
					break;
				case SQLTokenizer.MONEY:
					((Money) value).value = Math.min(((Money) value).value,
							expr.getMoney());
					break;
				case SQLTokenizer.TIMESTAMP:
				case SQLTokenizer.SMALLDATETIME:
				case SQLTokenizer.DATE:
				case SQLTokenizer.TIME:
					((DateTime) value).time = Math.min(((DateTime) value).time,
							expr.getLong());
					break;
				default:
	 ...
	 * 
	 * @param expr
	 *            the expression that produce the values which should be summary
	 * @throws Exception
	 */
	 ...
	 *            The new Value.
	 * @param newDataType
	 *            The data type of the new Value (One of the SQLTokenizer
	 *            const). If the type is -1 then the data type is verify with
	 *            many instanceof expressions.
	 * @throws SQLException
	 *             If the newValue is not a instance of a know class.
	 */
	final void set(Object newValue, int newDataType) throws SQLException {
		...
		if (dataType < 0) {
			...
			else if (newValue instanceof Long || newValue instanceof Identity)
	...
	static boolean getBoolean(Object obj, int dataType) throws Exception {
		...
		default:
			return Utils.string2boolean(obj.toString());
		}
	}
		...
		default:
			...
			try {
				return Integer.parseInt(str);
									 ...
									 * A NumberFormatException can occur if it a
	...
	static long getMoney(Object obj, int dataType) throws Exception {
		...
		default:
			return Money.parseMoney(obj.toString());
		}
	}
	...
	static MutableNumeric getNumeric(Object obj, int dataType) {
		...
		default:
			return new MutableNumeric(obj.toString());
		}
	}
	...
	static byte[] getBytes(Object obj, int dataType) throws Exception {
		...
		default:
			throw createUnsupportedConversion(dataType, obj,
	...
	static SQLException createUnsupportedConversion(int fromDataType,
			Object obj, int toDataType) {
		Object[] params = { SQLTokenizer.getKeyWord(fromDataType), obj,
				SQLTokenizer.getKeyWord(toDataType) };

		return SmallSQLException
				.create(Language.UNSUPPORTED_CONVERSION, params);
	}
